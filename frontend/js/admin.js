...
async function loadDashboardData() {
  ...
  try {
    ...
    const designsData = designsResponse.ok ? await designsResponse.json() : { data: [] };
  ...
  tbody.innerHTML = filtered.length
    ? filtered.map(user => `
      <tr>
        ...
        <td><span class="badge ${user.role === 'admin' ? 'badge-awaiting' : 'badge-muted'}">${escapeHtml(user.role)}</span></td>
